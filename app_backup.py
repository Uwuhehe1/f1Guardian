...
def admin_login():
    """Log in with ENV admin (ADMIN_USER/ADMIN_PASS) OR a DB admin (username/email + password)."""
    error = None
    if request.method == "POST":
        login_value = (request.form.get("username") or "").strip()
        password = request.form.get("password") or ""
        if login_value == ADMIN_USER and password == ADMIN_PASS:
...
def admin_users_list():
    ...
    try:
        # NOTE: template can call _is_protected_admin(u) thanks to jinja_env.globals set above
...
def admin_users_create():
    if request.method == "POST":
        ...
        msg = ("Email already exists." if res.get("error") == "email_exists"
        ...
        try:
            return render_template("admin_user_new.html", error=msg, name=name, username=username, email=email, role=role)
        except Exception:
...
def trigger_model_retrain():
    TRAIN_CSV = os.getenv("TRAIN_CSV", p("data", "training.csv"))
    try:
        pipe = train_and_save(TRAIN_CSV, MODEL_PATH, threshold=ML_THRESHOLD)
        global model
...
def analyze_url(url: str):
    ...
    # ---------------- Base domain-level checks (your existing signals) ----------------
    if any(bad in d for bad in SUSPICIOUS_DOMAINS):
        score += 30; reasons.append("domain matches suspicious list")
    if rd in SHORTENERS:
        score += 12; reasons.append("URL shortener")
    if is_ip_link(nurl):
        score += 25; reasons.append("IP address in host")
    if any(is_similar(d, legit) and d != legit for legit in OFFICIAL_DOMAINS):
        score += 18; reasons.append("lookalike to official domain")

    # Stronger punycode rule
    if looks_like_punycode(d):
        score += 40; reasons.append("punycode in domain")
    elif has_non_ascii(d):
        score += 10; reasons.append("non-ASCII chars in domain")

    # TLD / subdomain / port
    tld = d.split('.')[-1] if '.' in d else ''
    if tld in SUSPICIOUS_TLDS:
        score += 25; reasons.append(f"suspicious TLD .{tld}")

    if excessive_subdomains(d):
        score += 8; reasons.append("excessive subdomains")

    host = (parsed.netloc or '').lower()
    if ':' in host:
        port = host.rsplit(':', 1)[-1]
        if port and port.isdigit() and port in SUSPICIOUS_PORTS:
            score += 8; reasons.append(f"suspicious port :{port}")

    host_and_path = parsed.netloc + parsed.path + ('?' + parsed.query if parsed.query else '')
    if '@' in host_and_path:
        score += 10; reasons.append("'@' in URL (credential-hiding trick)")

    if count_percent_encoded(nurl) >= 3:
        score += 6; reasons.append("heavy percent-encoding")

    if len(nurl) > 100:
        score += 4; reasons.append("unusually long URL")
    ...
    'duckdns.org', 'ath.cx', 'is-a.dev', 'is-a.fun', 'is-an.app',
    ...
    if rd in HOSTING_RDS:
        ...
        # ignore the hosting platform's own brand, focus on others
        brands_in_url = [b for b in brands_in_url if BRAND_KEYWORDS[b] != rd]
        if brands_in_url:
            score += 18
            reasons.append(f"brand bait on generic hosting ({rd}): {', '.join(sorted(set(brands_in_url)))}")

    
    if any(k in url_l for k in URL_KEYWORDS):
        score += 10; reasons.append("phishing keywords in path/query")
    if any(k in url_l for k in URL_KEYWORDS_POLISH):
        score += 10; reasons.append("phishing keywords (PL) in path/query")

    
    ppath = parsed.path or ''
    if len(ppath) > 30 and shannon_entropy(ppath) > 3.5:
        score += 6; reasons.append("high-entropy path")

    
    mism = brand_mismatch(d, nurl)
    if mism:
        score += 30; reasons.append(f"brand/domain mismatch: {', '.join(mism)}")
    ...
    if any(ppath.lower().endswith(ext) for ext in SUSP_EXTS) and any(k in url_l for k in URL_KEYWORDS):
        score += 12; reasons.append("login file extension with credential wording")

    
    if scheme == 'http:' and any(k in url_l for k in URL_KEYWORDS):
        score += 6; reasons.append("HTTP with credential keywords")
    ...
    try:
        ...
        if labels and labels[0] not in ('www','m'):
            ...
            if len(left) >= 12 and shannon_entropy(left) > 3.3:
                score += 8; reasons.append("randomized subdomain label")
    except Exception:
        pass
    ...
    if len(present_brands) >= 2:
        score += 12; reasons.append(f"multiple brand names in URL: {', '.join(present_brands[:4])}")
...
def check_links(urls):
    ...
    for url in urls:
        ...
        total += score
        if score > 0:
            details.append({'url': url, 'score': score, 'reasons': reasons})
...
def detect_with_scoring(email_text):
    ...
    matched, fb_reasons = _matches_feedback(email_text)
    if matched:
        details = (
            "<strong>Risk Score: n/a (user feedback)</strong><br>"
            "ML Probability: n/a (user override)<br>"
            "<br><strong>Reasons (feedback):</strong><br>" + "<br>".join(f"• {r}" for r in fb_reasons)
        )
        return {
            'tp': True,'detection_type': "TP",'warning': True,'risk_color': 'red',
            'result': "True Positive: Previously user-marked phishing.",
            'detection_reason': "; ".join(fb_reasons),'details': details
        }
    ...
    hard_tp = False
    tp_reasons = []
    if has_credential_harvest_link(normalized_text):
        hard_tp = True; tp_reasons.append("Critical credential-harvest link")
    if not hard_tp and high_kws and (has_tld or has_cred_path):
        hard_tp = True; tp_reasons.append("High-risk keywords + suspicious link cues")
    if not hard_tp and mal_hits and any(w in lower for w in ("login","password","verify","account","reset")):
        hard_tp = True; tp_reasons.append("Malware indicator + credential language")
    if not hard_tp and (has_puny and (has_cred_path or max_link_score >= STRONG_LINK_THRESHOLD)):
        hard_tp = True; tp_reasons.append("Punycode domain + credential/link risk")

    
    if not hard_tp and ml_prob >= ML_THRESHOLD:
        ml_corroborated = (
            risk_score >= REVIEW_RISK_THRESHOLD or
            has_cred_path or
            max_link_score >= STRONG_LINK_THRESHOLD or
            has_tld or has_puny or
            bool(high_kws) or bool(mal_hits)
        )
        if ml_corroborated:
            hard_tp = True
            tp_reasons.append("ML threshold met + corroborating indicators")
        else:
            detection_type = "Review"
            risk_color = "yellow" if risk_score >= SAFE_RISK_THRESHOLD else "green"
            result = "Review: ML flagged but no corroborating risk signals."
            detection_reason = f"ML {ml_prob:.2f} ≥ {ML_THRESHOLD:.2f} without heuristic corroboration"
            warning = False
    ...
    if hard_tp:
        ...
        result = f"False negative: Phishing detected ({'; '.join(tp_reasons)})."
        detection_reason = "; ".join(tp_reasons) + (" + malware indicators" if mal_hits else "")
        warning = True
    else:
        never_safe = []
        if has_cred_path:            never_safe.append("credential path")
        if has_tld:                  never_safe.append("suspicious TLD")
        if max_link_score >= STRONG_LINK_THRESHOLD: never_safe.append(f"high link score {max_link_score}")
        if mal_hits:                 never_safe.append("malware indicator")
        if has_puny:                 never_safe.append("punycode domain")
        if obfuscation_present(email_text): never_safe.append("obfuscation present")

        if mal_hits:
            detection_type = "FN"; risk_color = 'red'
            result = "False Negative: Malware indicator detected."
            detection_reason = "Malware keywords/extension in email"; warning = True
            ...
            detection_reason = "Credential path + suspicious link"; warning = True
        elif never_safe:
            detection_type = "Suspicious"; risk_color = 'orange'
            result = "Suspicious: Risk indicators present (ML below threshold)."
            detection_reason = "Floor rule: " + ", ".join(never_safe); warning = False
            ...
            result = "Near miss: Strong phishing signals but ML below threshold."
            detection_reason = (f"Signals: high_kw={bool(high_kws)}, susp_tld={has_tld}, "
            ...
            if risk_score >= SUSP_RISK_THRESHOLD:
                detection_type = "Suspicious"; risk_color = 'red'
                ...
                result = ("Suspicious: Moderate risk indicators (ML below threshold)."
                          if detection_type == "Suspicious" else "Low risk indicators detected — review carefully.")
                detection_reason = ("Moderate risk, but ML below threshold" if detection_type == "Suspicious"
                                    else "Keyword/TLD floor to Review")
            elif risk_score >= SAFE_RISK_THRESHOLD:
                detection_type = "Review"; risk_color = 'yellow'
                result = "Low risk indicators detected — review carefully."
                detection_reason = "Low risk indicators present"
            else:
                detection_type = "Safe"; risk_color = 'green'
...
def count_by_period() -> dict:
    ...
    for entry in data:
        if not entry.get('phishing'): continue
        ts = _safe_parse_ts(entry.get('timestamp', ''))
        if not ts: continue
        if ts.date() == now.date(): daily += 1
        if ts.isocalendar()[1] == now.isocalendar()[1] and ts.year == now.year: weekly += 1
        if ts.year == now.year: yearly += 1
